        ...
        {activityEvents.map((event) => (
          <article key={event.id} className="flex gap-4 rounded-[16px] border border-[var(--color-border)] bg-white p-5 shadow-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[14px] leading-6 text-[var(--color-text-secondary)]">{event.detail}</p>
              {event.actor && <div className="mt-2 text-[12px] font-bold text-[var(--color-accent)]">{event.actor}</div>}
            </div>
          </article>
        ))}
